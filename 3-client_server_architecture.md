...
__Версии HTTP:__
0.9 самая первая; 1.1 самая распространенная; 2.0 самая современная.
...
__Идемпотентные методы.__ Свойство операции, при повторном применении дает тот же результат, что и при первом.
- GET
- HEAD
- OPTIONS
- TRACE
...
## Методы HTTP запросов
- __GET.__ Запрашивает ресурс. Безопасный. Идемпотентный. Не имеет тело запроса, но имеет тело ответа.
- __POST.__ Передача необработанных данных на сервер. Логин и пароль. Небезопасный. Не идемпотентный. Имеет тело запроса и ответа.
- __PUT.__ Заменяет все текущие представления ресурса данными запроса. Небезопасный. Идемпотентный. Имеет тело запроса, но не имеет тело ответа.
- __DELETE.__ Удаляет указанный ресурс. Небезопасный. Идемпотентный. Может содержать тело запроса и ответа.
- __HEAD.__ Запрашивает ресурс. Возвращает заголовки. Безопасный. Идемпотентный. Не имеет тело запроса и ответа. Аналогичен методу GET.
- __PATCH.__ Аналогичен PUT, но для передачи фрагмента данных. Небезопасный. Не идемпотентный. Имеет тело запроса и ответа. Частично изменяет ресурс.
- __OPTIONS.__ Показывают какие HTTP-методы доступны. Безопасный. Идемпотентный. Не имеет тело запроса, но имеет тело ответа.
- __TRACE.__ Возвращает полученный запрос так, что клиент может увидеть, какую информацию промежуточные серверы добавляют или изменяют в запросе. Безопасный. Идемпотентный. Не имеет тело запроса и ответа.
- __CONNECT.__ Устанавливает «Туннель» к серверу, определённому по ресурсу. Запускает двухстороннюю связь. Небезопасный. Не идемпотентный. Не имеет тело запроса, но имеет тело ответа.
...
## VPN и Proxy разница
__VPN__. Виртуальная приватная сеть. Состоит из проксей. Не может существовать без прокси. Работает на уровне операционной системы

- __Плюсы:__ Защита данных. Анонимность в сети. Обход ограничений интернет-провайдера.

- __Минусы:__ Задержка сети (пинг). Снижение пропускной способности сети. Медленнее.
...
- __Идентификация__ - процесс нахождения логина пользователя в БД. Проводится при сопоставлении логина;
...
---
- __IP-адрес__ – уникальный идентификатор адреса сети. IP означает «Интернет-протокол» – набор правил, регулирующих формат данных. Состоит из четырех чисел (в диапазоне от 0 до 255), которые разделены точками. На смену __IPv4__ идет протокол __IPv6__. 
	- __Статический IP__ - все четыре числа неизменны
	- __Динамический IP__ - числа могут меняются
